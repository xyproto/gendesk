...
func writeDesktopFile(pkgname string, name string, comment string, exec string,
	categories string, genericName string, mimeTypes string) {
	...
	for _, pkgname := range pkgnames {
		...
		if verbose {
			fmt.Printf("%s%s%s%s%s ",
				darkGrayText("["), lightBlueText(pkgname),
				darkGrayText("]"), spaces,
				darkGrayText("Generating desktop file..."))
		}
		writeDesktopFile(pkgname, name, comment, exec,
			categories, genericName, mimeType)
		if verbose {
			fmt.Printf("%s\n", darkGreenText("ok"))
		}

		// Download an icon if it's not downloaded by
		// the PKGBUILD and not there already
		...
		if ((len(files) == 0) && (iconurl == "")) && (*nodownload == false) {
			...
			if err == nil {
				...
				if verbose {
					...
					fmt.Printf("%s%s%s%s%s ",
						darkGrayText("["),
						lightBlueText(pkgname),
						darkGrayText("]"),
						spaces,
						darkGrayText("Using default icon instead..."))
				}
