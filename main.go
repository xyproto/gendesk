...
func (vars Variables) CreateWindowManagerDesktopContents() *bytes.Buffer {
	var (
		buf bytes.Buffer
		wm  = WMStarter{vars[NAME], vars[EXEC]}
	)
...
func (vars Variables) CreateDesktopContents(useTerminal, startupNotify bool, categories, mimeTypes []string) *bytes.Buffer {
	var (
		buf bytes.Buffer
		app = AppStarter{vars[NAME], vars[GENERICNAME], vars[COMMENT], vars[EXEC], vars[ICON], vars[PATH], useTerminal, startupNotify, strings.Join(categories, ";"), strings.Join(mimeTypes, ";")}
	)
	// Inserting strings into the template should always work, panic if not
...
func (vars Variables) WriteWindowManagerDesktopFile(force bool, o *textoutput.TextOutput) {
	buf := vars.CreateWindowManagerDesktopContents()
	if vars[CUSTOM] != "" {
		// Write the custom string to the end of the .desktop file (may contain \n)
		buf.WriteString(vars[CUSTOM] + "\n")
	}
	// Check if the file exists (and that force is not enabled)
	if _, err := os.Stat(vars[PKGNAME] + ".desktop"); err == nil && (!force) {
		o.Err("no")
		o.Fprintln(os.Stderr, vars[PKGNAME]+".desktop already exists. Use -f as the first argument to overwrite it.")
		os.Exit(1)
	}
	ioutil.WriteFile(vars[PKGNAME]+".desktop", buf.Bytes(), 0644)
}

// Write the .desktop file as generated by createDesktopContents
func (vars Variables) WriteDesktopFile(useTerminal, startupNotify, force bool, o *textoutput.TextOutput) {
	...
	if vars[ICON] == "" {
		vars[ICON] = vars[PKGNAME]
	}

	// mimeTypes may be empty. Disabled terminal
	// and startupnotify for now.
	buf := vars.CreateDesktopContents(useTerminal, startupNotify, categoryList, mimeTypeList)
	if custom := vars[CUSTOM]; custom != "" {
		// Write the custom string to the end of the .desktop file (may contain \n)
...
func main() {
	...
	// If arguments was given, check if one of them is a PKGBUILD
	if a := args[0]; a != "" {
		filename := ""
		if exists(a) {
			filename = a
		} else {
			maybeFilename := filepath.Join(vars.Get("SRCDEST", "../"), "PKGBUILD")
			if exists(maybeFilename) {
				filename = maybeFilename
			}
		}
		if filename != "" {
			fmt.Println("READ VARIABLES FROM FILENAME INTO VARS HERE:", filename)
		}
	}
	...
	//vars.DataFromEnvironment(&pkgdesc, execCommand, name, genericname, mimetypes, comment, categories, custom)

	// Expand all variables, using simple variable expansion
	//vars.ExpandAll()
	...
	if filename != "" {
		...
		if !exists(filename) {
			...
			vars.Set("pkgname", pkgname)
		} else {
			// TODO: Use a struct per pkgname instead
			vars.ParsePKGBUILD(o, filename, &iconurl, &pkgname, &pkgnames, &pkgdescMap, &execMap, &nameMap, &genericNameMap, &mimeTypesMap, &commentMap, &categoriesMap, &customMap)
		}
	}
	...
	fmt.Println("All variables from the environment, after parsing PKGBUILD:")
	for k, v := range *vars {
		fmt.Printf("\t%s:\t%s\n", k, v)
	}
	...
	for _, pkgname := range pkgnames {
		...
		vars.Set("pkgname", pkgname)
		vars.Set("pkgdesc", pkgdesc)
		vars.Set("_name", name)
		vars.Set("_exec", execCommand)
		vars.Set("_custom", custom)
		vars.Set("_comment", comment)
		vars.Set("_icon", *icon)
		vars.Set("_path", *path)
		vars.Set("_categories", categories)
		vars.Set("_genericname", genericName)
		vars.Set("_mimetypes", mimeTypes)

		// Two level variable expansion
		vars.ExpandAll()
		vars.ExpandAll()

		if *windowmanager {
			vars.WriteWindowManagerDesktopFile(*force, o)
		} else {
			vars.WriteDesktopFile(*terminal, *startupnotify, *force, o)
		}
