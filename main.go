...
func writeWindowManagerDesktopFile(pkgname, name, execCommand, custom string, force bool, o *textoutput.TextOutput) {
	buf, err := createWindowManagerDesktopContents(name, execCommand)
	if err != nil {
		o.Err("no")
		o.Fprintf(os.Stderr, "internal error when executing template: %v\n", err)
		os.Exit(1)
	}
	if custom != "" {
	...
	if exists(pkgname+".desktop") && !force {
		o.Err("no")
		o.Fprintf(os.Stderr, "%s.desktop already exists. Use -f as the first argument to overwrite it.\n", pkgname)
		os.Exit(1)
	}
...
func writeDesktopFile(pkgname, name, comment, execCommand, icon, path, categories, genericName, mimeTypes, custom string, useTerminal, startupNotify, force bool, o *textoutput.TextOutput) {
	...
	buf, err := createDesktopContents(name, genericName, comment, execCommand, icon, path, useTerminal, startupNotify, categoryList, mimeTypeList)
	if err != nil {
		o.Err("no")
		o.Fprintf(os.Stderr, "internal error when executing template: %v\n", err)
		os.Exit(1)
	...
	filename := pkgname + ".desktop"

	// Check if the file exists (and that force is not enabled)
	if exists(filename) && !force {
		o.Err("no")
		o.Fprintf(os.Stderr, "%s already exists. Use -f as the first argument to overwrite it.\n", filename)
		os.Exit(1)
	}
