...
func createDesktopContents(name, genericName, comment, exec, icon, path string, useTerminal, startupNotify bool, categories, mimeTypes []string) *bytes.Buffer {
	var (
		buf bytes.Buffer
		app = AppStarter{name, genericName, comment, exec, icon, path, useTerminal, startupNotify, strings.Join(categories, ";"), strings.Join(mimeTypes, ";")}
	)
	// Inserting strings into the template should always work, panic if not
...
func writeDesktopFile(pkgname, name, comment, exec, icon, path, categories, genericName, mimeTypes, custom string, useTerminal, startupNotify, force bool, o *textoutput.TextOutput) {
	...
	buf := createDesktopContents(name, genericName, comment, exec, icon, path, useTerminal, startupNotify, categoryList, mimeTypeList)
	if custom != "" {
		// Write the custom string to the end of the .desktop file (may contain \n)
		buf.WriteString(custom + "\n")
	}
...
func main() {
	...
	for _, pkgname := range pkgnames {
		...
		if *windowmanager {
			...
			writeDesktopFile(pkgname, name, comment, exec, *icon, *path, categories, genericName, mimeTypes, custom, *terminal, *startupnotify, *force, o)
		}
